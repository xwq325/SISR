...
def join(image_dir, model_name):
    ...
    for image_file in image_files:
        ...
        # y = 30
        # fill = (0, 255, 0)
        # draw.text((x, y), "({}) {}".format(ch[index - 1], model_name[index - 1]), font=font, fill=fill)
        # index = index + 1
        images.append(image)
    ...
    while i >= 0:
        ...
        i = i - 1
    # 创建一个新的白色背景图像
    new_image = Image.new('RGB', (3 * width + 20, 2 * height + 10), 'white')

    # 将四个图像粘贴到新图像的正确位置
    new_image.paste(images[0], (0, 0))
    new_image.paste(images[1], (10 + width, 0))
    new_image.paste(images[2], (20 + 2 * width, 0))
    # new_image.paste(images[3], (30 + 3 * width, 0))
    new_image.paste(images[3], (0, 10 + height))
    new_image.paste(images[4], (10 + width, 10 + height))
    new_image.paste(images[5], (20 + 2 * width, 10 + height))
    # new_image.paste(images[7], (30 + 3 * width, 10 + height))
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(input_path):
        if filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            enlarge_drawing(args.x, args.y, image_name=filename, savepath=save_path, image=image, right=args.right,
                            save=args.save_partial)
