...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        if self.args.load != '' and self.args.resume != 0:
            if self.args.resume > 0:
                resume_epoch = int(self.args.resume)
            elif self.args.resume == -2:
                if hasattr(self.ckp, 'log') and torch.is_tensor(self.ckp.log) and self.ckp.log.ndim == 4 and self.ckp.log.size(0) > 0:
                    best_idx = int(self.ckp.log.max(0)[1][0, 0, 0].item()) + 1
                    resume_epoch = best_idx
                else:
                    resume_epoch = len(self.ckp.log)
            elif self.args.resume == -1:
                resume_epoch = len(self.ckp.log)
            self.optimizer.load(ckp.dir, epoch=resume_epoch, resume=self.args.resume)
    ...
    def train(self):
        ...
        # TEMP
        idx_scale = random.randint(0, len(self.scale) - 1)
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                self.ckp.log[-1, idx_data, idx_scale, 0] /= len(d)
                ...
                self.ckp.write_log(
                    '[{} x{}]\tPSNR: {:.3f} SSIM: {:.5f} (Best: {:.3f} @epoch {})'.format(
                        ...
                        best[0][idx_data, idx_scale, 0],
                        best[1][idx_data, idx_scale, 0] + 1
                    )
                )
